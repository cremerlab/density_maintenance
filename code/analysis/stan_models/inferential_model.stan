data {
    ...
    //--------------------------------------------------------------------------
    int<lower=1> J_growth; // Number of unique conditions 
    int<lower=1> J_growth_curves; // Number of unique growth curves
    int<lower=1> N_growth_meas; // Total number of points
    array[N_growth_meas] int<lower=1, upper=J_growth_curves> growth_curve_idx;
    array[J_growth_curves] int<lower=1, upper=J_growth> growth_cond_idx;
    vector<lower=0>[N_growth_meas] growth_time;
    ...
    //--------------------------------------------------------------------------
    //  Literature Growth Measurements.
    //--------------------------------------------------------------------------
...
model { 
    ...
    // Priors
    width_mu ~ std_normal();
    width_mu[J_size_wt_idx] ~ normal(width_min + width_slope .* growth_mu[J_growth_wt_idx], width_lam_sigma);
    width_sigma ~ std_normal();
    length_mu ~ normal(0, 2);
    length_mu[J_size_wt_idx] ~ normal(length_min + length_slope .* growth_mu[J_growth_wt_idx], length_lam_sigma);
