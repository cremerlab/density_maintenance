...
parameters { 
    ...
    vector<lower=0>[J_peri] log_od595nm_peri_mu;
    vector<lower=0>[J_peri] log_od595nm_peri_sigma;
...
model { 
    ...
    log(mem_od562nm) ~ normal(log_od562nm_mem_mu[mem_idx] + log(mem_conv_factor), log_od562nm_mem_sigma[mem_idx]);

    // Periplasmic measurement modelo
    log_od595nm_peri_mu ~ std_normal();
    log_od595nm_peri_sigma ~ std_normal();
    log(peri_od595nm) ~ normal(log_od595nm_peri_mu[peri_idx] + log(peri_conv_factor), log_od595nm_peri_sigma[peri_idx]);
...
generated quantities {
    ...
    vector<lower=0>[N_prot] protein_per_cell_ppc;
    ...
    for (i in 1:N_mem) {
        mem_ppc[i] = exp(normal_rng(log(mem_conv_factor[i]) + log_od562nm_mem_mu[mem_idx[i]], log_od562nm_mem_sigma[mem_idx[i]]));
    }
    ...
    for (i in 1:N_peri) {
        peri_ppc[i] = exp(normal_rng(log(peri_conv_factor[i]) + log_od595nm_peri_mu[peri_idx[i]], log_od595nm_peri_sigma[peri_idx[i]]));
    }
    ...
    // Protein per cell ppc
    for (i in 1:N_prot) {
        protein_per_cell_ppc[i] = exp(normal_rng(log_protein_intercept + log_protein_slope * prot_growth_rate[i], log_protein_slope));
    }
