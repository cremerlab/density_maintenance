...
model = cmdstanpy.CmdStanModel(
    stan_file='./stan_models/inferential_model.stan')
...
# Valid conditions
strains = ['wildtype', 'malE-rbsB-fliC-KO']
overexpression = ['none', 'malE']
inducer_conc = [0, 10, 50, 100]
...
size_data = size_data[size_data['strain'].isin(strains) &
                      size_data['overexpression'].isin(overexpression) &
                      size_data['inducer_conc'].isin(inducer_conc)]
...
growth_data = growth_data[growth_data['strain'].isin(strains) &
...
growth_data['cond_idx'] = growth_data.groupby(
    ['strain', 'carbon_source', 'overexpression', 'inducer_conc']).ngroup() + 1
growth_data['curve_idx'] = growth_data.groupby('run_idx').ngroup() + 1
for g, d in size_data.groupby(['strain', 'carbon_source', 'overexpression', 'inducer_conc', 'size_cond_idx']):
...
data_dict = {
    ...
    'J_growth_curves': growth_data['curve_idx'].max(),
    'N_growth_meas': len(growth_data),
    'growth_cond_idx': growth_data.groupby(['curve_idx'])['cond_idx'].min().astype(int),
    'growth_curve_idx': growth_data['curve_idx'].values.astype(int),
    ...
    'growth_size_idx': growth_data.groupby(['cond_idx'])['cond_idx'].min().astype(int),
