...
fig, ax = plt.subplots(1, 3, figsize=(3, 2.5), sharey=True)
...
for i, prop in enumerate(['width_mu', 'aspect_ratio_mu', 'rho_ratio']):
    ...
    ax[i].fill_between(q['value'], 2 * np.ones(len(q)),  2 +
                       q['kde'] / q['kde'].max(), lw=0.5, color=cor['pale_purple'], alpha=0.75)


for i, prop in enumerate(['width_mu', 'aspect_ratio_mu', 'rho_ratio']):
    q = malE_oe[malE_oe['parameter'] == prop]
    ax[i].plot(q['value'], 1 + q['kde'] / q['kde'].max(), '-',
               lw=0.5, color=cor['primary_green'])
    ax[i].fill_between(q['value'], 1 * np.ones(len(q)),  1 +
                       q['kde'] / q['kde'].max(), lw=0.5, color=cor['pale_green'], alpha=0.75)
...
for i, prop in enumerate(['width_mu', 'aspect_ratio_mu', 'rho_ratio']):
    ...
    ax[i].fill_between(q['value'],  np.zeros(len(q)),
                       q['kde'] / q['kde'].max(), lw=0.5, color=cor['pale_black'], alpha=0.75)
