data { 
    ...
    int<lower=1> N_vol; // Number of volume measurements data points (for lit MS measurements)
    int<lower=1> N_sa; // Number of surface area measurements data points (for lit MS measurements)
    ...
    vector<lower=0>[N_prot] lit_prot;
    vector<lower=0>[N_vol] vol_growth_rate_hr;
    vector<lower=0>[N_vol] lit_volume;
    vector<lower=0>[N_sa] sa_growth_rate_hr;
    vector<lower=0>[N_sa] lit_sa;
...
transformed data {
    ...
    vector<lower=0>[N_prot] log_lit_prot;
    vector<lower=0>[N_vol] log_lit_volume;
    vector<lower=0>[N_sa] log_lit_sa;
}
...
model {
   ...
   log_lit_prot ~ normal(beta_0_prot + beta_1_prot * prot_growth_rate_hr, prot_sigma);
   log_lit_volume ~ normal(beta_0_vol + beta_1_vol * vol_growth_rate_hr, vol_sigma);
   log_lit_sa ~ normal(beta_0_sa + beta_1_sa * sa_growth_rate_hr, sa_sigma);
}
