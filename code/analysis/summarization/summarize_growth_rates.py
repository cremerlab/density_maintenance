...
for g, d in tqdm.tqdm(data.groupby(['strain', 'carbon_source',  'overexpression', 'inducer_conc_ng_ml', 'run_idx'])):
    ...
    slope = popt[0]
    intercept = popt[1]
    _df = pd.DataFrame([np.array([g[0], g[1], g[2], g[3], g[4], slope, np.exp(intercept)])],
                       columns=['strain', 'carbon_source', 'overexpression', 'inducer_conc_ng_ml', 'run_idx', 'growth_rate_hr', 'od_init'])
