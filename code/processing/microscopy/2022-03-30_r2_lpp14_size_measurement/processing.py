...
for i, f in enumerate(tqdm.tqdm(files)):
    ...
    carbon = f.split('/')[-1].split(f'{STRAIN}_')[1].split('-')[0][:-1]
...
# %%
cell_sizes.to_csv(
    './output/{DATE}_r{RUN_NO}_{STRAIN}_cell_sizes.csv', index=False)
cell_splines.to_csv(
    './output/{DATE}_r{RUN_NO}_{STRAIN}_cell_splines.csv', index=False)
# %%
