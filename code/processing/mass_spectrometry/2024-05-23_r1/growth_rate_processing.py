...
for g, d in df.groupby(['strain', 'inducer_conc', 'replicate', 'axis']):
    ...
    ax[g[3]].plot(d['elapsed_time_hr'], d['od_600nm'], 'o', ms=4, color=cor['primary_black'])
    ax[g[3]].plot(time_range, fit, '-', color=cor['primary_red'], lw=1)
    ax[g[3]].set_title(f"{g[0]}, Ind{g[1]} r{g[2]}: $\lambda$={_fit['growth_rate_hr'].values[0]:0.2f} inv.hr.", fontsize=6)
for a in ax:
    a.set_yscale('log')
plt.tight_layout()
