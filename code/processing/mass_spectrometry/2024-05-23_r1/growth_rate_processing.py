# %%
"""
Note that this script drops samples for pRelA with 4 ng/mL of dox. The samples were
not harvested and thus the growth rate is not used.
"""
import numpy as np
import pandas as pd
...
data = data[~((data['strain'] == 'relA') & (data['inducer_conc'] == 4))]
# %%
# Calculate the time in minutes since the start of the experiment
df = pd.DataFrame([])
for g, d in data.groupby(['replicate', 'strain', 'inducer_conc']):
    d = d.copy()
...
for g, d in df.groupby(['strain', 'inducer_conc', 'replicate']):
    popt = scipy.stats.linregress(
        d['elapsed_time_hr'].values, np.log(d['od_600nm'].values))
...
df['axis'] = df.groupby(['strain', 'inducer_conc', 'replicate']).ngroup()
n_cols = 3
...
for g, d in df.groupby(['strain', 'inducer_conc', 'replicate', 'axis']):
    _fit = stats_df[(stats_df['strain'] == g[0]) &
                    (stats_df['inducer_conc'] == g[1]) &
                    (stats_df['replicate'] == g[2])]
    time_range = np.linspace(0, d['elapsed_time_hr'].max(), 100)
    fit = _fit['od_init'].values[0] * \
        np.exp(_fit['growth_rate_hr'].values[0] * time_range)
    ax[g[3]].plot(d['elapsed_time_hr'], d['od_600nm'],
                  'o', ms=4, color=cor['primary_black'])
    ax[g[3]].plot(time_range, fit, '-', color=cor['primary_red'], lw=1)
    ax[g[3]].set_title(
        f"{g[0]}, Ind{g[1]} r{g[2]}: $\lambda$={_fit['growth_rate_hr'].values[0]:0.2f} inv.hr.", fontsize=6)
for a in ax:
    a.set_yscale('log')
plt.tight_layout()
