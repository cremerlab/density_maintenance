...
for i in range(1, 3):
    for j, strain in enumerate(STRAINS):
        for k, ind in enumerate(INDUCERS):
            files = glob.glob(f'./raw/images/*{strain}*_{ind}_rep_{i}*.tif')
            if len(files) == 0:
                continue
            ...
            fname = f'./viz/{DATE}_{strain}_ind_{ind}_rep{i}_gallery.png'
            suptitle=f'{strain} induction {ind} replicate {rep}'
