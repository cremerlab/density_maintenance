...
agged_ms_rel = agged_ms.pivot(index=keys[:-1], columns='localization', values='rel_mass_frac').reset_index()
...
#%%
for ag, nm in zip([agged_ms, agged_ms_rel], ['', '_relative']):
    merged = ag.merge(size_data, on=['date', 'strain', 'carbon_source', 'inducer_conc', 'replicate'])
    ...
    merged.to_csv(f'./total_collated_data{nm}.csv', index=False)
