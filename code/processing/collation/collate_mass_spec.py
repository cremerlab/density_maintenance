...
cog_mapper = {g[0]:[g[1], g[2]] for g, _ in lit_data.groupby(['gene_name', 'cog_class', 'cog_letter'])}
...
for g, d in merged_data.groupby('gene_name'):
    if g not in cog_mapper.keys():
        print(f'{g} not in cog_mapper')
        unmapped += 1
        continue
    cog_class, cog_letter = cog_mapper[g]
