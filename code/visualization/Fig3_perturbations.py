...
for g, d in posts[posts['quantity'].isin(list(pars.keys()))].groupby(['quantity', 'overexpression', 'inducer_conc']):
    ...
    _ = _ax.hist(d['value'], bins=100, color=axes[g[1]][g[2]][1],
...
for g, d in wt_data.groupby(['carbon_source']):
    ...
    for i, _d in enumerate([sav, w]):
        ...
        ax[i].vlines(phi['median_value'], _d['2.5%'], _d['97.5%'],
                     lw=1, color=cor['primary_black'], alpha=0.75)
        ax[i].plot(phi['median_value'], _d['median_value'], 'o', markerfacecolor='w',
...
for g, d in params.groupby(['overexpression', 'inducer_conc']):
    ...
    for i, (_d, _c) in enumerate(zip([sav, w], ['red', 'blue'])):
        ...
        ax[i].hlines(_d['median_value'], phi['2.5%'],
                     phi['97.5%'], lw=1, color=_cor)
        ax[i].vlines(phi['median_value'], _d['2.5%'],
                     _d['97.5%'], lw=1, color=_cor)
        ax[i].plot(phi['median_value'], _d['median_value'], colors[g[0]]['m'], markerfacecolor='w',
...
plt.savefig('../../figures/Fig3.1_theory_comparison.pdf')
