...
theo = pd.read_csv('../../data/mcmc/theory_phiRb_prediction_summaries.csv') wt = pd.read_csv('../../data/mcmc/wildtype_posterior_parameter_summaries.csv')
...
for g, d in ms_data.groupby('dataset_name'):
    ...
    ax[0].plot(phiRb['mass_frac'] / 0.4558, phi_mem['mass_frac'], **fmt, ms=5)
    ax[1].plot(phiRb['mass_frac'] / 0.4558, phi_peri['mass_frac'], **fmt, ms=5)
...
for i, p in enumerate([phi_mem_relation, phi_peri_relation]):
    for g, d in p.groupby('interval', sort=False):
        ax[i].fill_between(d['phiRb'] / 0.4558, d['lower'], d['upper'], alpha=0.5,
...
for g, d in wt.groupby(['carbon_source']):
    ...
    for i, p in enumerate([phi_mem, phi_peri]):
        if len(p) > 0:
            ax[i].hlines(p['median_value'], phiRb['2.5%'] / 0.4558,
                         phiRb['97.5%'] / 0.4558, lw=1, color=cor['primary_black'])
            ax[i].vlines(phiRb['median_value'] / 0.4558, p['2.5%'],
                         p['97.5%'], lw=1, color=cor['primary_black'])
            ax[i].plot(phiRb['median_value'] / 0.4558,  p['median_value'], 'o',
...
for g, d in sav.groupby('interval', sort=False):
    ax.fill_between(d['phiRb'] / 0.4558, d['lower'], d['upper'], alpha=0.75, color=cor[f'{inter_colors[g]}green'])
...
for g ,d in wt.groupby('carbon_source'):
    ...
    ax.hlines(sav['median_value'], phi['2.5%']/0.4558, phi['97.5%']/0.4558, lw=1, color=cor['primary_black'])
    ax.vlines(phi['median_value']/0.4558, sav['2.5%'], sav['97.5%'], lw=1, color=cor['primary_black'])
