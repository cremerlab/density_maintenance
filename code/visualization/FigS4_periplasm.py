...
ax[1].set_ylim([0, 20])
ax[1].set_ylabel('periplasmic protein\nmass [fg/cell]\n', fontsize=6)
ax[0].set_ylabel('total protein\nmass [fg/cell]', fontsize=6)
ax[1].set_xlabel('growth rate\n$\lambda$ [hr$^{-1}$]', fontsize=6)
ax[0].set_ylim([80, 1000])
ax[0].set_yscale('log')
ax[0].set_yticks([100, 250, 500, 1000])
ax[0].set_yticklabels([100, 250, 500, 1000])

_pars = pars[pars['quantity'] == 'm_peri']
ax[1].fill_between([0, 2.5], _pars['2.5%'].values * [1, 1], _pars['97.5%'].values
                   * [1, 1], color=cor['pale_purple'])
ax[1].hlines(_pars['median_value'], 0, 2.5, lw=1,
             color=cor['purple'])
...
for g, d in ms_emp[ms_emp['quantity'] == 'ms_m_peri'].groupby('source'):
    ...
    ax[1].vlines(d['growth_rate_hr'], d['2.5%'], d['97.5%'],
...
plt.savefig('../../figures/FigS4_periplasm_protein_trends.pdf')
# %%
...
plt.savefig('../../figures/FigS4_periplasm_protein_posteriors.pdf',
            bbox_inches='tight')
...
axes = {'phi_peri_lam_pred': ax[0], 'rho_peri_pred': ax[1]}
for g, d in pred[pred['quantity'].isin(['phi_peri_lam_pred', 'rho_peri_pred'])].groupby(['quantity', 'interval'], sort=False):
    axes[g[0]].fill_between(d['growth_rate_hr'], d['lower'], d['upper'],
                            color=interval_colors[g[1]], alpha=0.5, zorder=1000)
...
#                    markeredgecolor=cor['primary_blue'], markeredgewidth=1, ms=4)
plt.tight_layout()
plt.savefig('../../figures/FigS4_periplasm_allocation_density_prediction.pdf',
            bbox_inches='tight')
