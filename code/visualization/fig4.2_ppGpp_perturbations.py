...
axes[-1, 0].set_xlabel('ribosomal proteome fraction\n$\phi_{rib}$', fontsize=6)
...
for a in ax:
    ...
    fmt['alpha'] = 0.5
    fmt['markersize'] = 4
    a.vlines(wt_data['measured_SAV'], wt_data['predicted_SAV_2sig_lower'], 
             wt_data['predicted_SAV_2sig_upper'], lw=0.5,
              color=cor['pale_black'], alpha=0.5)
    a.vlines(wt_data['measured_SAV'], wt_data['predicted_SAV_1sig_lower'], 
             wt_data['predicted_SAV_1sig_upper'], lw=1,
              color=cor['pale_black'], alpha=0.5)
...
for g, d in rel.groupby(['carbon_source', 'inducer_conc']):
    fmt = {'marker':markers[g[0]],
           ...
           'linestyle':'none', 
           'alpha': 0.75}
    ax[1].vlines(d['measured_SAV'], d['predicted_SAV_2sig_lower'], 
                 d['predicted_SAV_2sig_upper'], lw=0.5,
              color=cor['dark_red'])
    ax[1].vlines(d['measured_SAV'], d['predicted_SAV_1sig_lower'], 
                 d['predicted_SAV_1sig_upper'], lw=1,
              color=cor['dark_red'])
...
for g, d in rel.groupby(['carbon_source', 'inducer_conc']):
    fmt = {'marker':markers[g[0]],
           ...
           'linestyle':'none',
           'alpha': 0.75}
    ax[0].vlines(d['measured_SAV'], d['predicted_SAV_2sig_lower'], 
                 d['predicted_SAV_2sig_upper'], lw=0.5,
              color=cor['dark_gold'])
    ax[0].vlines(d['measured_SAV'], d['predicted_SAV_1sig_lower'], 
                 d['predicted_SAV_1sig_upper'], lw=1,
              color=cor['dark_gold'])
